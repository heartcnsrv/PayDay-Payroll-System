...
/*
 * Removes unwanted characters from a CSV token.
 * Cleans spaces, quotes, and newline characters before storing data.
 */
static void strip(char* s) {
    ...
    /* Remove leading spaces or quotes */
    char* p = s;
    while (*p == ' ' || *p == '"') p++;

    /* Shift cleaned string to the start */
    if (p != s) memmove(s, p, strlen(p) + 1);
}

/*
 * Parses a CSV line into tokens.
 * Handles quoted values so commas inside quotes are not split.
...
/*
 * Loads employee records from CSV file into memory.
 * Skips the header row and converts each line into an Employee struct.
 */
int csv_load_employees(const char* path, Employee* out, int max) {
    ...
    while (fgets(line, sizeof(line), f) && count < max) {
        ...
        if (n < 12) continue; /* Ensure valid data */

        /* Convert tokens into Employee struct */
...
/*
 * Saves all employee records into CSV file.
 * Overwrites the file and writes a header first.
 */
...
/*
 * Generates next employee ID (auto-increment simulation).
 * Finds highest ID and returns max + 1.
 */
...
/*
 * Loads payroll records from CSV into memory.
 * Each row becomes one PayrollRecord struct.
 */
int csv_load_payroll(const char* path, PayrollRecord* out, int max) {
    ...
    while (fgets(line, sizeof(line), f) && count < max) {
        ...
        /* Convert tokens into PayrollRecord struct */
        ...
        if (n > 17)
            strncpy(p->status, tok[17], sizeof(p->status)-1);
        else
            strncpy(p->status, "generated", sizeof(p->status)-1);
    }
...
/*
 * Saves all payroll records into CSV file.
 * Overwrites existing file and writes header first.
 */
...
/*
 * Generates next payroll ID (auto-increment simulation).
 */
 ...
 * Used for consistent date storage in CSV.
 */
